...
export const RenderComponent = () => ({
  render(h) {
    return h(MyButton, { props: { color: "pink" } }, [
...
export const VuexActions = () => ({
  components: { MyButton },
  template:
    '<my-button @click="log">with vuex: {{ $store.state.count }}</my-button>',
...
export const WhateverYouWant = () => ({
  components: { MyButton },
  template:
    '<my-button @click="log">with awesomeness: {{ $store.state.count }}</my-button>',
  store: new Vuex.Store({
    state: { count: 0 },
    mutations: {
      increment(state) {
