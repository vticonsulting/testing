<template>
  <div>
    <button
      ...
      aria-haspopup="true"
      data-toggle="true"
      :class="['inline-block h-8 w-8 self-center scale-90 hover:scale-100 hover:text-white focus:outline-none focus:shadow-outline transform', { 'alerts-open': alertsOpen }]"
    >
      <slot>
        <svg
          class="h-8 w-8 stroke-current fill-current stroke-2 transform duration-500 ease-in-out"
          viewBox="0 0 24 24"
          fill="none"
...
<script>
import AppAlerts from '../AppAlertsMenu'

export default {
  name: 'AppAlertsMenuToggle',
  components: {
    AppAlerts,
  },
  props: {
